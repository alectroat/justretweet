 ...
 */
 class Myphirehose extends Phirehose{
	  ...
	  public function initialize() {
		$CI  =& get_instance();
		$CI->load->helper("myphirehose");
	  }
	  
	  public function enqueueStatus($status){
		
		saveJsonTweet($status);
		// Set your time limit as timestamp
		if( (time() - $this->connect_time) > 900) { $this->disconnect(); echo 'Timeout'; exit; };
	  }
 }
...
abstract class Phirehose
{
  ...
  */
  ...
  public function __construct($config = array())
  {
	...
	$this->connect_time = $config['connect_time'];
	$this->method   = isset($config['method']) ? $config['method'] : (Phirehose::METHOD_FILTER);
    $this->format   = isset($config['format']) ? $config['format'] : (self::FORMAT_JSON);
  }

...

{
	...
	public function index(){
			...
			$limit = 20;
			$total_result = $this->tweet_model->getAllVideosForAdmin(1000,1,$status)->num_rows();//$this->db->count_all('retweets');
			...
			$data['youtube_list'] = $this->tweet_model->getAllVideosForAdmin($limit,$start_ret,$status);
	...
	function delete() 
	{
		if($this->session->userdata('admin_id'))
		{
			...
			$this->tweet_model->deleteTube($this->uri->segment(5));
			
			redirect('admin/youtube/index/'.$status.'/'.$page);
		}
		else
		{
			redirect("admin/login");
	...
	function multidelete()
	{
		if($this->session->userdata('admin_id'))
		{
			...
			for($i=0; $i<$count; $i++)
			{
				$this->tweet_model->deleteTube($checkbox[$i]);
			}
			redirect("admin/youtube/index/".$status.'/'.$page);
		}
		else
	...
	function status()
	{
		if($this->session->userdata('admin_id'))
		{
			...
			$this->tweet_model->changeYoutubeStatus($this->uri->segment(5));

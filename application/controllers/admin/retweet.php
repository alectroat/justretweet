...

{
	...
	public function index(){
			...
			$limit = 20;
			$total_result = $this->tweet_model->getAllRetweetsForAdmin(1000,1,$status)->num_rows();//$this->db->count_all('retweets');
			...
			$data['retweet_list'] = $this->tweet_model->getAllRetweetsForAdmin($limit,$start_ret,$status);
			...
			$this->load->view("admin/retweet",$data);	   
	...
	function edit() 
	{
		if($this->session->userdata('admin_id'))
		{
			...
			if($_POST['submit']){
				...
				redirect('admin/retweet/index/'.$status.'/'.$page);
	...
	function delete() 
	{
		if($this->session->userdata('admin_id'))
		{
			...
			$this->tweet_model->deleteTweet($this->uri->segment(5));
	...
	function multidelete()
	{
		if($this->session->userdata('admin_id'))
		{
			...
			for($i=0; $i<$count; $i++)
			{
				$this->tweet_model->deleteTweet($checkbox[$i]);
			}
			redirect("admin/retweet/index/".$status.'/'.$page);
		}
		else
	...
	function status()
	{
		if($this->session->userdata('admin_id'))
		{
			...
			$this->tweet_model->changeTweetStatus($this->uri->segment(5));

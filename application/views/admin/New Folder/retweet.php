...
<link type="text/css" href="<?= base_url()?>asset/css/smoothness/ui.css" rel="stylesheet" />  
	...
	{
	window.location = "<?=base_url();?>admin/retweet/index/"+status+"/"+1;
	}
	
	function deleteTweet(tweet_id,page)
	{
		var con=confirm("Do You Really Want to Delete This!");
		if(con){
		var status = document.getElementById('s_status').value;
		window.location = '<?php echo site_url();?>admin/retweet/delete/'+status+'/'+tweet_id+'/'+page;
		}
	}
	...
	function doSearch(page){
	        var status = document.getElementById('s_status').value;
			window.location = "<?=base_url();?>admin/retweet/index/"+status+"/"+page;
		}
	</script>
...
<!-- CONTENT START -->
    ...
    <div class="portlet">
        ...
        <form action="<?php echo site_url();?>admin/retweet/multidelete/<?= $status ?>/<?= $current_page ?>" method="post" onsubmit="return selectcheckbox();">
		<input type="hidden" name="total" id="total" value="<?= $retweet_list->num_rows() ?>" />
		<input type="hidden" name="s_status" id="s_status" value="<?=$status?>" />
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
			<?php 
			if($retweet_list->num_rows()>0) {
			?>            
			<thead>
              <tr>
				...
				<th width="300" scope="col">Retweet</th>
			...
			<?php
			$i = 0;
			foreach($retweet_list->result() as $list): 
			?>
              <tr>
			  <td colspan="6">
			  <div style="float:left;width:30px;margin-top:15px"> <input type="checkbox" name="checkbox[]" id="check<?= $i ?>" value="<?= $list->tweet_id ?>" /></div>
			  ...
			  <div style="margin-top:25px;width:705px"><?=addAcronym($list->tweet); ?></div>
			  </div>
			  <div style="float:left;width:70px;margin-top:15px">
			  <a href="<?php echo site_url();?>admin/retweet/status/<?=$status?>/<?=$list->tweet_id?>/<?=$current_page?>" <?php if($list->status=='1'){ ?> class="active_icon"<?php }else{ ?> class="inactive_icon" <?php } ?> title="Change Status"></a>
			  <a href="<?php echo site_url();?>admin/retweet/edit/<?=$status?>/<?=$list->tweet_id?>/<?=$current_page?>" class="edit_icon" title="Edit This"></a>
			  <a href="#" class="delete_icon" onclick="deleteTweet('<?= $list->tweet_id ?>','<?= $current_page ?>')" title="Delete This"></a></div>

...
<link type="text/css" href="<?= base_url()?>asset/css/smoothness/ui.css" rel="stylesheet" />  
	...
	{
	window.location = "<?=base_url();?>admin/blog/index/"+status+"/"+1;
	}
	
	function deleteBlog(blog_id,page)
	{
		var con=confirm("Do You Really Want to Delete This!");
		if(con){
		var status = document.getElementById('s_status').value;
		window.location = '<?php echo site_url();?>admin/blog/delete/'+status+'/'+blog_id+'/'+page;
		}
	}
	...
	function doSearch(page){
			var status = document.getElementById('s_status').value;
			window.location = "<?=base_url();?>admin/blog/index/"+status+"/"+page;
		}
	</script>
...
<!-- TABS START -->
    <div id="tabs">
         <div class="container">
            <ul>
                      <li><a href="<?php echo site_url();?>admin/user/" class="current"><span>Blog List</span></a></li>
           </ul>
        </div>
    </div>
<!-- TABS END -->    
...
<!-- CONTENT START -->
    ...
    <!--  TITLE START  --> 
    <div class="grid_9">
    <h1 class="dashboard">Blog List</h1>
    </div>
    <div class="clear">
    </div>
    <!--  TITLE END  -->    
    ...
    <div class="portlet">
        ...
        <form action="<?php echo site_url();?>admin/blog/multidelete/<?= $status ?>/<?= $current_page ?>" method="post" onsubmit="return selectcheckbox();">
		<input type="hidden" name="total" id="total" value="<?= $blog_list->num_rows() ?>" />
          ...
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
			...
			<thead>
              <tr>
				...
				<th width="300" scope="col">Blogs</th>
			...
			<?php
			$i = 0;
			foreach($blog_list->result() as $list): 
			?>
              <tr>
			  <td colspan="6">
			  <div style="float:left;width:30px;margin-top:15px"> <input type="checkbox" name="checkbox[]" id="check<?= $i ?>" value="<?= $list->blog_id ?>" /></div>
			  ...
			  <div style="margin-top:25px;width:705px"><?=addAcronym($list->blog_url); ?></div>
			  </div>
			  <div style="float:left;width:70px;margin-top:15px">
			  <a href="<?php  echo site_url();?>admin/blog/status/<?= $status ?>/<?= $list->blog_id ?>/<?= $current_page ?>" <?php if($list->status=='1'){ ?> class="active_icon"<?php }else{ ?> class="inactive_icon" <?php } ?> title="Change Status"></a>
			  <a href="<?php echo site_url();?>admin/feed/<?= $list->blog_id ?>" class="view_icon" title="View Feeds"></a>
			  <a href="#" class="delete_icon" onclick="deleteBlog('<?= $list->blog_id ?>','<?= $current_page ?>')" title="Delete This"></a></div>

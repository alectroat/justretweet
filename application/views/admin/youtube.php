...
<script src="<?=base_url();?>asset/js/popups.js" type="text/javascript" charset="utf-8"></script>

<script language="javascript" type="text/javascript">

function sortByStatus(status)
	{
	window.location = "<?=base_url();?>admin/youtube/index/"+status+"/"+1;
	}

function ViewYoutube(video_id)
	{
	document.getElementById('contactArea').innerHTML='<object type=\"application/x-shockwave-flash\" style=\"width:425px; height:350px;\" data=\"http://www.youtube.com/v/'+video_id+'\"><param name=\"movie\" value=\"http://www.youtube.com/v/'+video_id+'\" /></object>';
	loadPopup();
	}

	
	function deleteYoutube(stream_id,page)
	{
		var con=confirm("Do You Really Want to Delete This!");
		if(con){
		var status = document.getElementById('s_status').value;
		window.location = '<?php echo site_url();?>admin/youtube/delete/'+status+'/'+stream_id+'/'+page;
		}
	}
	...
	function doSearch(page){
	        var status = document.getElementById('s_status').value;
			window.location = "<?=base_url();?>admin/youtube/index/"+status+"/"+page;
		}
	</script>
...
<!-- TABS START -->
    <div id="tabs">
         <div class="container">
            <ul>
                      <li><a href="<?php echo site_url();?>admin/user/" class="current"><span>YouTube List</span></a></li>
           </ul>
        </div>
    </div>
<!-- TABS END -->    
...
<!-- CONTENT START -->
    ...
    <!--  TITLE START  --> 
    <div class="grid_9">
    <h1 class="dashboard">YouTube List</h1>
    </div>
    <div class="clear">
    </div>
    <!--  TITLE END  -->    
    ...
    <div class="portlet">
        ...
        <form action="<?php echo site_url();?>admin/youtube/multidelete/<?= $status ?>/<?= $current_page ?>" method="post" onsubmit="return selectcheckbox();">
		<input type="hidden" name="total" id="total" value="<?=$youtube_list->num_rows()?>" />
		<input type="hidden" name="s_status" id="s_status" value="<?=$status?>" />
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
			<?php 
			if($youtube_list->num_rows()>0) {
			?>            
			<thead>
              <tr>
				...
				<th width="300" scope="col">Video</th>
			...
			<?php
			$i = 0;
			foreach($youtube_list->result() as $list): 
			?>
              <tr>
			  <td colspan="6">
			  <div style="float:left;width:30px;margin-top:15px"> <input type="checkbox" name="checkbox[]" id="check<?= $i ?>" value="<?= $list->stream_id ?>" /></div>
			  <div style="float:left;width:85px"><img src="<?=$list->profile_image?>"/></div>
			  <div style="float:left;width:705px">
			  <div><div style="float:left;width:435px">&nbsp;</div>
				  <div style="float:left;width:100px">By : <?= rtrim($list->account,'000NA') ?></div>
				  <div style="float:left;width:170px"><!--Posted on : <?php echo date("M jS Y", $list->timestamp); ?>--></div></div>
			  <div style="margin-top:25px;width:705px"><!--<font style="text-decoration:underline">Feed Title</font> :--><img width="50px" height="50px" src="<?=$list->video_thumbnail?>" /></div>
			  </div>
			  <div style="float:left;width:70px;margin-top:15px">
			  <a href="javascript:void(0)" class="video_icon" onclick="ViewYoutube('<?=$list->video_id?>');" title="Play video"></a>
			  <a href="<?php echo site_url();?>admin/youtube/status/<?=$status?>/<?=$list->stream_id?>/<?=$current_page?>" <?php if($list->status=='ON'){ ?> class="active_icon"<?php }else{ ?> class="inactive_icon" <?php } ?> title="Change Status"></a>
			  <a href="#" class="delete_icon" onclick="deleteYoutube('<?= $list->stream_id ?>','<?= $current_page ?>')" title="Delete This"></a></div>
		...
		<div id="popupContact" style="width:425px">

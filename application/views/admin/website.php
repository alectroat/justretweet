...
<link type="text/css" href="<?= base_url()?>asset/css/smoothness/ui.css" rel="stylesheet" />  
	...
	{
	window.location = "<?=base_url();?>admin/website/index/"+status+"/"+1;
	}
	
	function deleteWebsite(website_id,page)
	{
		var con=confirm("Do You Really Want to Delete This!");
		if(con){
		var status = document.getElementById('s_status').value;
		window.location = '<?php echo site_url();?>admin/website/delete/'+status+'/'+website_id+'/'+page;
		}
	}
	...
	function doSearch(page){
	        var status = document.getElementById('s_status').value;
			window.location = "<?=base_url();?>admin/website/index/"+status+"/"+page;
		}
	</script>
...
<!-- TABS START -->
    <div id="tabs">
         <div class="container">
            <ul>
                      <li><a href="<?php echo site_url();?>admin/user/" class="current"><span>Website List</span></a></li>
           </ul>
        </div>
    </div>
<!-- TABS END -->    
...
<!-- CONTENT START -->
    ...
    <!--  TITLE START  --> 
    <div class="grid_9">
    <h1 class="dashboard">Website List</h1>
    </div>
    <div class="clear">
    </div>
    <!--  TITLE END  -->    
    ...
    <div class="portlet">
        ...
        <form action="<?php echo site_url();?>admin/website/multidelete/<?= $status ?>/<?= $current_page ?>" method="post" onsubmit="return selectcheckbox();">
		<input type="hidden" name="total" id="total" value="<?= $website_list->num_rows() ?>" />
		<input type="hidden" name="s_status" id="s_status" value="<?=$status?>" />
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
			<?php 
			if($website_list->num_rows()>0) {
			?>            
			<thead>
              <tr>
				...
				<th width="300" scope="col">Website Url</th>
			...
			<?php
			$i = 0;
			foreach($website_list->result() as $list): 
			?>
              <tr>
			  <td colspan="6">
			  <div style="float:left;width:30px;margin-top:15px"> <input type="checkbox" name="checkbox[]" id="check<?= $i ?>" value="<?= $list->website_id ?>" /></div>
			  ...
			  <div style="margin-top:25px;width:705px"><?=addAcronym($list->url); ?></div>
			  </div>
			  <div style="float:left;width:70px;margin-top:15px">
			   <a href="<?php echo site_url();?>admin/website/getFeatured/<?=$status?>/<?=$list->website_id?>/<?=$current_page?>" <?php if($list->isFeatured=='1'){ ?> class="featured_icon"<?php }else{ ?> class="unfeatured_icon" <?php } ?> title="Get featured"></a>
			  <a href="<?php echo site_url();?>admin/website/status/<?=$status?>/<?=$list->website_id?>/<?=$current_page?>" <?php if($list->status=='1'){ ?> class="active_icon"<?php }else{ ?> class="inactive_icon" <?php }?> title="Change Status"></a>
			  <!--<a href="<?php echo site_url();?>admin/website/edit/<?=$status?>/<?=$list->website_id?>/<?=$current_page?>" class="edit_icon" title="Edit This"></a>-->
			  <a href="#" class="delete_icon" onclick="deleteWebsite('<?= $list->website_id ?>','<?= $current_page ?>')" title="Delete This"></a></div>
